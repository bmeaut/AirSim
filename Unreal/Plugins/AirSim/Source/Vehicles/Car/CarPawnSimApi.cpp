	...
	if (rc_data.is_initialized) {
		...
		if (rc_data.vendor_id == "VID_044F") {
			...
			if (carpawn != nullptr) {
				...
				float damper_strength = DAMPERGAIN * std::min(1.0, (std::abs(speedkmh) / 70.0));
				...
				if (hitUtilities_ == nullptr || !hitUtilities_->IsHitPhysicalEffectOn()) {
					...
					float autocenter_strength = std::min(1.0, (std::abs(speedkmh) / 20.0)) * std::sqrt(std::abs(joystick_controls_.steering / 4.0)) * steeringSign * AUTOCENTERGAIN;
